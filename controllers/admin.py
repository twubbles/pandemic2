...
@auth.requires_membership('admins')
def testdata():
    db((db.auth_user.id==db.game_part.user_id) & (db.game_part.game_id==request.args(0))).select(
...
@auth.requires_membership('admins')
def manage_users():
    users = db((db.auth_user.id)).select(db.auth_user.id, db.auth_user.first_name, db.auth_user.last_name, db.auth_user.handle, groupby=db.auth_user.id,cache=(cache.ram, 1),cacheable=True)
    ...
    if request.args(0):
        parts = db((db.auth_user.id==db.game_part.user_id) & (db.game_part.game_id==db.games.id) &
                   ...
                   (db.game_part.user_id==request.args(0))).select(db.games.game_name, db.game_part.id,
                    db.auth_user.first_name, db.auth_user.last_name, db.auth_user.handle, db.game_part.bitecode,
                    db.squads.name, db.game_part.squad_id, db.creature_type.name,
                    db.game_part.zombie_expires_at, db.auth_user.id, db.creature_type.zombie, db.creature_type.immortal,
                    groupby=db.game_part.id,cache=(cache.ram, 1),cacheable=True)
...
def sassy_posts():
    response.view='default.html'
    grid = SQLFORM.grid(db.sassypost, csv=False,searchable=False,sortable=False,create=True)
    return dict(form=grid)
...
@auth.requires_membership('admins')
def missionlist():
    grid = SQLFORM.grid(db.missions, csv=False,searchable=False,sortable=False,create=True)
    return dict(form=grid)
...
def ozlist():
    if request.args(0):
        gid = request.args(0)
        users=db((db.auth_user.id==db.game_part.user_id) & (db.game_part.game_id==request.args(0)) & (db.game_part.creature_type==db.creature_type.id) & (db.game_part.original_request==True)).select(
                  db.auth_user.first_name, db.auth_user.last_name, db.auth_user.handle, db.creature_type.name,
                  db.game_part.zombie_expires_at, db.creature_type.immortal,db.game_part.id)
        games=False
...
def regrequestlist():
    regreq=db((db.registration_request.user_id==db.auth_user.id) & (db.registration_request.game_id==db.games.id) & (db.registration_request.reviewed==False)).select(
                db.auth_user.id,db.auth_user.first_name,db.auth_user.last_name,db.auth_user.handle,db.games.id,db.games.game_name,
                db.auth_user.registration_id,db.registration_request.id,db.registration_request.reviewed,db.registration_request.approved,
